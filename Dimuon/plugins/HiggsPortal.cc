// -*- C++ -*-
//
// Package:    GenStudy/HiggsPortal
// Class:      HiggsPortal
// 
/**\class HiggsPortal HiggsPortal.cc GenStudy/HiggsPortal/plugins/HiggsPortal.cc
...
class HiggsPortal : public edm::EDAnalyzer {
public:
  explicit HiggsPortal(const edm::ParameterSet&);
  ~HiggsPortal();
...
private:
  ...
  // Dark fermion and dark photon histograms
  TH1F *h_darkFermionNum, *h_dFR;
  ...
  TH1F *h_dFPt, *h_lepJetPt, *h_dFLepPtDiff;
  TH1F *h_dFPhi, *h_lepJetPhi, *h_dFLepPhiDiff;
  TH1F *h_dFEta, *h_lepJetEta, *h_dFLepEtaDiff;
  TH1F *h_dFMass, *h_lepJetMass, *h_dFLepMassDiff;
...
void HiggsPortal::beginJob()
{
  edm::Service<TFileService> fs;
  // Dark fermion histograms
  h_darkFermionNum = fs->make<TH1F>("darkFermionNum", "Number of initial dark fermions per event", 14, 0, 14);
  h_dFR = fs->make<TH1F>("dFR", "delta r of the two initial dark fermions in the event", 100, 0, 10);
  ...
  h_darkPhotonEta = fs->make<TH1F>("darkPhotonEta", "Dark Photon eta", 100, -10, 10);
  ...
  h_lepR = fs->make<TH1F>("lepR", "Distance between a pair of two leptons that originated from the same dark photon in phi-eta space", 300, -0.1, 5.9);
  ...
  // Set of electrons that came from the same dark fermion histograms
  h_lepSetPT = fs->make<TH1F>("lepSetPT", "The total PT of a set of all the leptons that came from the same dark fermion", 100, 0, 400);
  h_lepSetE = fs->make<TH1F>("lepSetE", "The total energy of a set of all the leptons that came from the same dark fermion", 100, 0, 400);
  h_lepSetSigmaPhi = fs->make<TH1F>("lepSetSigmaPhi", "The standard deviation of the phis in a set of all the leptons that came from the same dark fermion", 100, -0.1, 3.2);
  h_lepSetSigmaEta = fs->make<TH1F>("lepSetSigmaEta", "The standard deviation of the etas in a set of all the leptons that came from the same dark fermion", 150, -.5, 2.5);
  ...
  // Probably meant neutralino
  h_actualJetNum = fs->make<TH1F>("actualJetNum", "The actual number of lepton jets in the event, based on whether neutralinos produced electrons", 15, -0.5, 14.5);
  ...
  // TODO: UNBREAK
  h_noJetDarkFermionNum = fs->make<TH1F>("noJetDarkFermionNum", "Each entry is an event with a dark fermion producing 4900002, which should have no decays (broken right now)", 0, 0, 1);
  ...
  // Four vector comparison histograms
  // Lepton jet only one per dark fermion
  h_dFPt = fs->make<TH1F>("dFPt", "The pt of all initial dark fermions in the event", 100, -100, 400);
  h_lepJetPt = fs->make<TH1F>("lepJetPt", "The pt of all lepton jets in the event", 100, -100, 400);
  h_dFLepPtDiff = fs->make<TH1F>("dFLepPtDiff", "The differnce in pt between all lepton jets and their dark fermions in the event", 100, -100, 400);

  h_dFPhi = fs->make<TH1F>("dFPhi", "The phi of all initial dark fermions in the event", 100, -3.2, 3.2);
  h_lepJetPhi = fs->make<TH1F>("lepJetPhi", "The phi of all lepton jets in the event", 100, -3.2, 3.2);
  h_dFLepPhiDiff = fs->make<TH1F>("dFLepPhiDiff", "The differnce in phi between all lepton jets and their dark fermions in the event", 100, -3.2, 3.2);

  h_dFEta = fs->make<TH1F>("dFEta", "The eta of all initial dark fermions in the event", 100, -10, 10);
  h_lepJetEta = fs->make<TH1F>("lepJetEta", "The eta of all lepton jets in the event", 100, -10, 10);
  h_dFLepEtaDiff = fs->make<TH1F>("dFLepEtaDiff", "The differnce in eta between all lepton jets and their dark fermions in the event", 100, -10, 10);

  h_dFMass = fs->make<TH1F>("dFMass", "The mass of all initial dark fermions in the event", 100, -100, 400);
  h_lepJetMass = fs->make<TH1F>("lepJetMass", "The mass of all lepton jets in the event", 100, -100, 400);
  h_dFLepMassDiff = fs->make<TH1F>("dFLepMassDiff", "The differnce in mass between all lepton jets and their dark fermions in the event", 100, -100, 400);
...
//
HiggsPortal::HiggsPortal(const edm::ParameterSet& iConfig)
...
void
HiggsPortal::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  using namespace edm;
  ...
  math::XYZTLorentzVectorD HiggsPortal;

  double numDarkPhotons = 0;
  double numDarkFermions = 0;
  ...
  const reco::Candidate* dF1;   // Variable to place first dark fermion while looking for the second
  const reco::Candidate* dF2;
  bool dF1Initialized = false;
  ...
  {
    // Dark fermion delta r calculations
    if(std::abs(part.pdgId()) == 4900001 && part.numberOfDaughters() > 1) // if particle (part) is a dark fermion and particle (part) has more than one daughter 
    {
      if(dF1Initialized)      // if the first dark fermion has been found and stored
      {
        ROOT::Math::LorentzVector d1 = dF1->p4();
	      dF2 = &part;
        ROOT::Math::LorentzVector d2 = dF2->p4();
	      h_dFR->Fill(reco::deltaR(d1, d2));
      }
      else // else save the first dark fermion
      {
	      dF1 = &part;
	      dF1Initialized = true;
      }
    ...
    if(part.pdgId() == 4900022) // if particle (part) is a dark photon
    {
      ...
      {
        const reco::Candidate* daughter1; // Container for final state leptons
    ...
    // starting from the dark fermion, find out how many lepton jets were produced
    // also store all the leptons from this jet into a collection for analysis
    // TODO: UNBREAK THIS PART
    std::vector<const reco::Candidate*> notLepSet;
    std::vector<const reco::Candidate*> lepSet;
    if(abs(part.pdgId()) == 4900001) // if particle is a dark fermion
    {
      if(part.numberOfDaughters() == 2 && part.daughter(0)->pdgId() != 4900001 && part.daughter(1)->pdgId() != 4900001) // if dark fermion has two daughters which aren't dark fermions
      {
        std::cout << "Dark Fermion Found" << std::endl;
	      numDarkFermions++;

        // Checking whether this particle creates a jet
	      if(part.daughter(0)->pdgId() != 4900002 && part.daughter(1)->pdgId() != 4900002) // if neither daughter is 4900002 (a 4900002 decay from dark fermion means no further particles will be produced)
	      {
	        actualJetNum++;
	      }
	      else if(part.daughter(0)->pdgId() == 4900002 || part.daughter(1)->pdgId() == 4900002) // if one of the dark fermion's decays is 4900002
      	{
          h_noJetDarkFermionNum->Fill(0);
        ...
        notLepSet.push_back(part.daughter(0)); // start with the two daughters of the dark fermion
  ...
  if(numDarkFermions != 0)
  {
    h_darkFermionNum->Fill(numDarkFermions); // fill the number of dark fermions for this event
  }
  else
  {
    std::cout << "No dark fermions in this event" << std::endl;
  }
  ...
  std::vector<const reco::Candidate*> lepsFromDF1;
  std::vector<const reco::Candidate*> lepsFromDF2;

  // comparing four vector of the lepton jet to the four vector of the dark fermions
  for(auto &lep : leptons)
  {
    const reco::Candidate* particleOfFocus =  lep;
    const reco::Candidate* motherOfFocus = lep->mother();

    int darkFermionPdgId = 4900001;
    bool darkFermionNotFound = true;
    while(darkFermionNotFound)
    {
      if(motherOfFocus->numberOfMothers() == 0)
      {
        darkFermionNotFound = false;
      }
      else if(motherOfFocus->pdgId() == darkFermionPdgId)
      {
        darkFermionNotFound = false;
        if(motherOfFocus == dF1)
        {
          lepsFromDF1.push_back(lep);
        }
        else if(motherOfFocus == dF2)
        {
          lepsFromDF2.push_back(lep);
        }
        else
        {
          std::cout << "four vector matdFng algorithm found a nonmatdFng dark fermion (initial)";
        }
      }
      else
      {
  ...
  ROOT::Math::LorentzVector dF1LepSum = dF1->p4() - dF1->p4();
  ROOT::Math::LorentzVector dF2LepSum = dF2->p4() - dF2->p4();
  for(auto &leps : lepsFromDF1)
  {
    dF1LepSum += leps->p4();
  }
  for(auto &leps : lepsFromDF2)
  {
    dF2LepSum += leps->p4();
  }

  if(!lepsFromDF1.empty())
  {
    h_dFPt->Fill(dF1->p4().pt());
    h_dFEta->Fill(dF1->p4().eta());
    h_dFPhi->Fill(dF1->p4().phi());
    h_dFMass->Fill(dF1->p4().mass());    

    h_lepJetPt->Fill(dF1LepSum.pt());
    h_lepJetEta->Fill(dF1LepSum.eta());
    h_lepJetPhi->Fill(dF1LepSum.phi());
    h_lepJetMass->Fill(dF1LepSum.mass());
    
    h_dFLepPtDiff->Fill(dF1->p4().pt() - dF1LepSum.pt());
    h_dFLepEtaDiff->Fill(dF1->p4().eta() - dF1LepSum.eta());
    h_dFLepPhiDiff->Fill(reco::deltaPhi(dF1->p4().phi(), dF1LepSum.phi()));
    h_dFLepMassDiff->Fill(dF1->p4().mass() - dF1LepSum.mass());


  }

  if(!lepsFromDF2.empty())
  {
    h_dFPt->Fill(dF2->p4().pt());
    h_dFEta->Fill(dF2->p4().eta());
    h_dFPhi->Fill(dF2->p4().phi());
    h_dFMass->Fill(dF2->p4().mass());

    h_lepJetPt->Fill(dF2LepSum.pt());
    h_lepJetEta->Fill(dF2LepSum.eta());
    h_lepJetPhi->Fill(dF2LepSum.phi());
    h_lepJetMass->Fill(dF2LepSum.mass());

    h_dFLepPtDiff->Fill(dF2->p4().pt() - dF2LepSum.pt());
    h_dFLepEtaDiff->Fill(dF2->p4().eta() - dF2LepSum.eta());
    h_dFLepPhiDiff->Fill(reco::deltaPhi(dF2->p4().phi(), dF2LepSum.phi()));
    h_dFLepMassDiff->Fill(dF2->p4().mass() - dF2LepSum.mass());
  }
...
int HiggsPortal::leptonJetReco(std::vector<const reco::Candidate*> leptons){
  double leptonJets = 0;
...
const reco::Candidate* HiggsPortal::findBiggestPT(std::vector<const reco::Candidate*> leptons, std::vector<const reco::Candidate*> &smallerPTLeptons){
  double biggestPT = 0;
...
const void HiggsPortal::printGenParticleCollection(const reco::GenParticleCollection& genParts)
{
...
const int HiggsPortal::getIndexOf(const reco::Candidate* part, const reco::GenParticleCollection& genParts)
{
  int indexOf = 0;
...
bool HiggsPortal::isBoson(int pid)
{
...
bool HiggsPortal::isMuon(int pid){
...
bool HiggsPortal::checkBosonStatus( const reco::GenParticleCollection& genParts){
...
const reco::Candidate* HiggsPortal::getBoson( const reco::GenParticleCollection& genParts)
{
  for(auto &part : genParts){
...
const reco::Candidate* HiggsPortal::getLastDaughter(const reco::Candidate* part,int pid)
{
...
const reco::Candidate* HiggsPortal::getDaughter(const reco::Candidate* part,int pid)
{  
  for(size_t partNr =0; part && partNr<part->numberOfDaughters();partNr++){
 ...
 const reco::Candidate* HiggsPortal::getMother(const reco::Candidate* part, int pid)
{
...
/*void 
HiggsPortal::beginRun(edm::Run const& iRun, edm::EventSetup const& iEventSetup)
{
...
void 
  HiggsPortal::endRun(edm::Run const& iRun, edm::EventSetup const& iEventSetup)
  {
   edm::Handle< GenRunInfoProduct > genInfoProduct;
  iRun.getByToken(genInfoProductToken_, genInfoProduct );
  crossSec = genInfoProduct->internalXSec().value();
...
/*
  void 
  HiggsPortal::beginLuminosityBlock(edm::LuminosityBlock const&, edm::EventSetup const&)
  {
  }
*/
...
/*
  void 
  HiggsPortal::endLuminosityBlock(edm::LuminosityBlock const&, edm::EventSetup const&)
  {
  }
*/
...
void
HiggsPortal::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
